...
def plot_running_avg(totalrewards):
  ...
  fname = os.path.join(os.getcwd(), "train_logs/avg_dqn_no_car_ra.png")
  plt.savefig(fname)
...
def transform(s):
    ...
    return bottom_black_bar_bw, upper_field_bw
...
def create_nn(model_to_load, stack_len):
    try:
        ...
        model = Sequential()
        model.add(Dense(512, input_shape=(stack_len*107,), kernel_initializer="lecun_uniform"))
...
class DQNAgent():
    def __init__(self, num_episodes, model_name=None, carConfig=None, replay_freq=20):
        ...
        self.model, self.init_weights = create_nn(model_name, self.stack_len)
        self.target_models = []
    ...
    def predict(self, s):
        return self.model.predict(np.reshape(s, (1, self.stack_len*107)), verbose=0)[0]

    def target_predict(self, s):
        total_pred = self.target_models[0].predict(np.reshape(s, (1, self.stack_len*107)), verbose=0)[0]
        for i in range(1, self.K):
            pred = self.target_models[i].predict(np.reshape(s, (1, self.stack_len*107)), verbose=0)[0]
            total_pred += pred
    ...
    def replay(self, batch_size):
        ...
        old_states = np.reshape(old_states, (batch_size, 107*self.stack_len))
    ...
    def play_one(self, eps,train=True,video_path=None):
        ...
        totalreward = 0
        iters = 0
        a, b = transform(observation)
        state = np.concatenate((np.array([compute_steering_speed_gyro_abs(a)]).reshape(1,-1).flatten(), b.reshape(1,-1).flatten()), axis=0) # this is 3 + 7*7 size vector.  all scaled in range 0..1      
        ...
        while not done:
            ...
            a, b = transform(observation)        
            curr_state = np.concatenate((np.array([compute_steering_speed_gyro_abs(a)]).reshape(1,-1).flatten(), b.reshape(1,-1).flatten()), axis=0) # this is 3 + 7*7 size vector.  all scaled in range 0..1      
    ...
    def train(self, retrain=False):
        ...
        for n in range(self.num_episodes):
            ...
            if n>=0 and n%100==0 and not self.model_name:
                # save model (assuming this is NOT the flask app, which WILL pass a model name)
                trained_model = os.path.join(os.getcwd(),"train_logs/avg_dqn_no_car_{}.h5".format(str(n)))
                with open("train_logs/avg_dqn_total_rewards_no_car_{}.pkl".format(str(n)),'wb+') as outfile:
        ...
        if not self.model_name:
            plt.plot(totalrewards)
            rp_name = os.path.join(os.getcwd(), "train_logs/avg_dqn_no_car.png")
            plt.title("Rewards")
            plt.savefig(rp_name)
            plt.close()
            plot_running_avg(totalrewards)
            with open(os.path.join(os.getcwd(), "train_logs/avg_dqn_total_rewards_final_no_car.pkl",'wb+')) as outfile:
                pkl.dump(totalreward, outfile)
        self.env.close()

if __name__ == "__main__":
    trainer = DQNAgent(3001, None, replay_freq=10)
    trainer.train()
