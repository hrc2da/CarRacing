...
def plot_running_avg(totalrewards):
  ...
  fname = os.path.join(os.getcwd(), "dqn_frame_skip_running_avg.png")
  plt.savefig(fname)
    ...
    def play_one(self, eps):
        ...
        while not done:
            ...
            if self.replay_freq==0:
                print("INVALID REPLAY FREQUENCY")
            if self.replay_freq!=0:
    ...
    def train(self, retrain=False):
        ...
        for n in range(self.num_episodes):
            ...
            if n>0 and n%50==0 and not self.model_name:
                # save model
                trained_model = os.path.join(os.getcwd(),"dqn_frame_skip_{}.h5".format(str(n)))
        ...
        if not self.model_name:
            ...
            rp_name = os.path.join(os.getcwd(), "dqn_frame_skip_rewards.png")
            plt.title("Rewards")
            plt.savefig(rp_name)
            plt.close()
...
if __name__ == "__main__":
    trainer = DQNAgent(1000, None, replay_freq=5)
    trainer.train()
