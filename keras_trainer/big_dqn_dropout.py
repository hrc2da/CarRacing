...
def plot_running_avg(totalrewards):
  ...
  fname = os.path.join(os.getcwd(), "dropout_running_avg.png")
  plt.savefig(fname)
...
def create_nn(model_to_load):
    try:
        ...
        model = Sequential()
        model.add(Dropout(0.4, input_shape=(4*111)))
    ...
    def train(self, retrain=False):
        ...
        for n in range(self.num_episodes):
            ...
            if n>0 and n%50==0 and not self.model_name:
                # save model
                trained_model = os.path.join(os.getcwd(),"dropout_trained_model_{}.h5".format(str(n)))
        ...
        if not self.model_name:
            ...
            rp_name = os.path.join(os.getcwd(), "dropout_rewards_big.png")
            plt.title("Rewards")
            plt.savefig(rp_name)
            plt.close()
