...
def create_nn(model_to_load, stack_len, config_len, freeze_hidden=False, lr = 0.01):
    try:
        ...
        model = Sequential()
        model.add(Dense(512, input_shape=(stack_len*111+config_len,), kernel_initializer="lecun_uniform"))# 7x7 + 3.  or 14x14 + 3 # a
...
class DQNAgent():
    def __init__(self, num_episodes, model_name=None, carConfig=None, replay_freq=20, freeze_hidden=False, lr=0.01, train_dir="train_logs_testing"):
        ...
        config_len = env.env.env.car.config_len
        ...
        for _ in range(self.K):
            target_model, _ = create_nn(model_name, config_len, self.stack_len)
    ...
    def play_one(self, eps,train=True,video_path=None):
        ...
        while not done:
            ...
            stacked_state = np.append(stacked_state, self.env.env.env.get_config_vector())
...
if __name__ == "__main__":
    ...
    trainer = DQNAgent(301, None, replay_freq=50, lr=0.001)#, carConfig=car_config)
    trainer.train()
