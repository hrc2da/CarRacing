...
def target_predict(inp):
    (ind, next_state) = inp
    print("predicting index: ", ind)
    target_model = target_models[ind]
    print("Target model index : ", ind, target_model)
    ret = target_model.predict(np.reshape(next_state, (1, 4*111)), verbose=0)[0] # remember to set the right number of stack frames
    print("Done predicting index: ", ind)
    return ret
...
class DQNAgent():
    def __init__(self, num_episodes, model_name=None, carConfig=None, replay_freq=20):
        ...
        for i in range(self.K):
            ...
            curr_target = target_models[i]
            curr_target.set_weights(self.init_weights)
            target_models[i] = curr_target
        self.model.summary()
    ...
    def update_targets(self):
        ...
        model_ind = self.curr_pointer%self.K
        model_to_change = target_models[model_ind]
        model_to_change.set_weights(model_weights)
        target_models[model_ind] = model_to_change
        self.curr_pointer += 1
    ...
    def replay(self, batch_size):
        ...
        for (old_state, argmax_qval, reward, next_state) in batch:
            ...
            with ThreadPoolExecutor(max_workers=8) as p:
                # res = p.map(self.target_predict, list(product(list(np.arange(self.K)), [next_state])))
                # res = p.map(self.target_predict, list(np.arange(self.K)))
                # print(self.target_models)
                # target_models_pkl = [tm.get_weights() for tm in self.target_models]
                res = p.map(target_predict, list(product(np.arange(self.K), [next_state])))
            print(list(res))
...
if __name__ == "__main__":
    ...
    target_models = []
    for _ in range(10): # number of networks in history
        target_model, _ = create_nn('', 4) #NOTE:again, change the stack length
