...
def create_nn(input_shape):        
    ...
    model.add(Conv1D(32, 2, strides=4,
                              padding='valid',
                              activation='relu',
                              input_shape=input_shape))

        # Third convolutional layer
    model.add(Conv1D(64, 1, strides=1,
                            padding='valid',
                            activation='relu',
                            input_shape=input_shape))
...
class DQNAgent():
    def __init__(self, env, num_episodes):
        self.env = env
        self.gamma = 0.99
        self.model = create_nn((4, 111))  # 4 consecutive steps, 111-element vector for each state
        MEMORY_SIZE = 10000
    ...
    def update(self, s, G, B):
        self.model.fit(np.reshape(s, (1, 4, 111)), np.array(G).reshape(-1, 11), batch_size=B, epochs=1, use_multiprocessing=True, verbose=0)
    ...
    def replay(self, batch_size):
        ...
        old_states = np.array(old_states)#reshape(batch_size, 40)
