...
class DQNAgent():
    ...
    def replay(self, batch_size):
        ...
        for (old_state, argmax_qval, reward, next_state) in batch:
            ...
            target = target.detach() #label, so no gradient needed
            if targets is None:
                targets = target
    ...
    def play_one(self, eps):
        ...
        while not done:
            ...
            if self.replay_freq!=0:
                if iters % self.replay_freq==0 and iters>self.replay_size:
                    try:
                        self.replay(64)
    ...
    def train(self, retrain=False):
        ...
        for n in range(self.num_episodes):
            ...
            if n>0 and n%500==0 and not self.model_name:
                # save model
                trained_model = os.path.join(os.getcwd(),"train_logs/avg_dqn_trained_model_torch_5_{}.h5".format(str(n)))
        ...
        if not self.model_name:
            ...
            rp_name = os.path.join(os.getcwd(), "train_logs/avg_dqn_10000_rewards_torch_5.png")
            plt.title("Rewards")
            plt.savefig(rp_name)
            plt.close()
...
if __name__ == "__main__":
    trainer = DQNAgent(5001, None, replay_freq=64)
    trainer.train()
