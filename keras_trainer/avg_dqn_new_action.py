...
def plot_running_avg(totalrewards):
  ...
  fname = os.path.join(os.getcwd(), "train_logs/avg_dqn_new_action_ra.png")
  plt.savefig(fname)
...
def create_nn(model_to_load, stack_len):
    try:
        ...
        model.add(Dense(21, kernel_initializer="lecun_uniform"))
...
class DQNAgent():
    ...
    def sample_action(self, s, eps):
        ...
        if np.random.random() < eps:
            return random.randint(0, 20), qval
        else:
    ...
    def convert_argmax_qval_to_env_action(self, output_value):
        ...
        elif output_value >= 9 and output_value <= 14:
            output_value -= 8
            gaz = float(output_value)/10 # 10-60% variable gas
        elif output_value >= 15 and output_value <= 20:
            output_value -= 14
            brake = float(output_value)/10 # 10-60% variable brakes
        else:
            print("error")        
    ...
    def replay(self, batch_size):
        ...
        for (old_state, argmax_qval, reward, next_state) in batch:
            ...
            old_state_preds.append(y.reshape(1, 21))
        old_states = np.reshape(old_states, (batch_size, 111*self.stack_len))
        old_state_preds = np.array(old_state_preds).reshape(batch_size, 21)
    ...
    def train(self, retrain=False):
        ...
        for n in range(self.num_episodes):
            ...
            if n>=0 and n%100==0 and not self.model_name:
                ...
                trained_model = os.path.join(os.getcwd(),"train_logs/avg_dqn_new_action_{}.h5".format(str(n)))
                with open("train_logs/avg_dqn_total_rewards_new_action_{}.pkl".format(str(n)),'wb+') as outfile:
        ...
        if not self.model_name:
            plt.plot(totalrewards)
            rp_name = os.path.join(os.getcwd(), "train_logs/avg_dqn_new_action.png")
            plt.title("Rewards")
            plt.savefig(rp_name)
            plt.close()
            plot_running_avg(totalrewards)
            with open(os.path.join(os.getcwd(), "train_logs/avg_dqn_total_rewards_final_new_action.pkl",'wb+')) as outfile:
