...
def reset_video_recorder_filename(filename,env):
    ...
    env.video_recorder = video_recorder.VideoRecorder(
                ...
                enabled=True#env._video_enabled(),
            )
...
class DQNAgent():
    def __init__(self, num_episodes, model_name=None, carConfig=None, replay_freq=20, freeze_hidden=False, lr=0.01, train_dir="train_logs",vid_path='flaskapp/static'):
        K.clear_session()
        env = gym.make('CarRacingRandomTrack-v1')
        env = wrappers.Monitor(env, vid_path, force=False, resume = True, video_callable=lambda x : True, mode='evaluation', write_upon_reset=False)
...
if __name__ == "__main__":
    ...
    trainer = DQNAgent(300, None, replay_freq=10, lr=0.001, train_dir="train_logs_random_track2")#, carConfig=car_config)
    trainer.train()
