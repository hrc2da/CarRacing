...
def plot_running_avg(totalrewards):
  ...
  fname = os.path.join(os.getcwd(), "bigdqn_3000_running_avg.png")
  plt.savefig(fname)
    ...
    def train(self, retrain=False):
        ...
        for n in range(self.num_episodes):
            ...
            if n>0 and n%50==0 and not self.model_name:
                # save model
                trained_model = os.path.join(os.getcwd(),"bigdqn_trained_model_{}.h5".format(str(n)))
        ...
        if not self.model_name:
            ...
            rp_name = os.path.join(os.getcwd(), "dqn_3000_rewards_big.png")
            plt.title("Rewards")
            plt.savefig(rp_name)
            plt.close()
