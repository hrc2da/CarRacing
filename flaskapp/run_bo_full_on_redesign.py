...
from utils import feature_labels, feature_ranges, car2config, config2car, densities, wheelmoment, blacklist, featuremask2names, perturb

n_design_steps = 30
notes = "I am adding 4 randomly perturbed samples of the redesign to increase it's influence"
# activate carracing_clean20
testing = False
#run on = 0 2 3 5 6 7 8 9
run_on = 4 # amit
...
base_config = car2config(session['final_design'],correct_negative_widths=True) ###!!!!!! THIS IS CHANGED!!!!!

new_session_dir = os.path.join(Config.FILE_PATH,user_id,"bo_sessions_all_features_start_from_redesign",str(session["_id"])) # session_id is an ObjectID, not str
...
for _ in range(4):
    perturbed_design = perturb(base_config, mask_indices)
    x0.append(perturbed_design)
    y0 = np.append(y0,test_drive(perturbed_design))
