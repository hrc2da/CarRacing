...
finished = list(experiments.find({"finished_training": {"$exists": True}, "final_test_drive_results": {"$exists": False}}))
...
for e in finished:
    ...
    for i in range(num_test_drives):
        ...
        driver.env.close()
    experiments.find_one_and_update({'_id':eid},{'$set':{'final_test_drive_vids':videos,'final_test_drive_results':results}})
