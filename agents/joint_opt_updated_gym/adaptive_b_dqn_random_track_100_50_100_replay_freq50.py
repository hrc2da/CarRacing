...
train_dir = 'updated_gym_train_logs_adaptive_25_50_40'
...
if __name__=="__main__":
    ...
    x0 = [config]
    y0 = np.array([result])
    rewards = policy_step(config2car(config),99)
    for i in range(25):  
        ...
        with open(f'{train_dir}/designs_{i}','wb+') as design_dump:
            pkl.dump([x0,y0],design_dump)
        # else:
            # x0.extend(results.x_iters)
            # np.concatenate((y0,results.func_vals))
            # import pdb; pdb.set_trace()
        rewards = policy_step(config2car(config),100)
