...
from platypus import GeneticAlgorithm, NSGAII, Problem, Type, Real, Binary, Integer,ProcessPoolEvaluator, PoolEvaluator,CompoundOperator,SBX,HUX,MultiprocessingEvaluator, run_job, Generator, RandomGenerator, Solution, InjectedPopulation
...
def np_arr2car(arr,agent=None):
    # this is less preferred because no constraints
    # pass in an nsgaii_agent so we don't have to instantiate every time
    if agent is None:
        agent = nsgaii_agent()
    assert len(arr) == agent.problem.nvars
    ranges = [(t.min_value, t.max_value) for t in agent.problem.types]
    car = np.zeros(agent.problem.nvars)
    for i,scalar in enumerate(arr):
        low,high = ranges[i]
        car[i] = scalar*(high-low) + low
        if type(agent.problem.types[i]) == Integer:
            car[i] = int(car[i])
    return agent.parse_config(agent.pack(car))

def generate_car(agent=None):
    if agent is None:
        agent = nsgaii_agent()
    car_sol = RandomGenerator().generate(agent.problem).variables
    car = [pt.decode(car_sol[i]) for i,pt in enumerate(agent.problem.types)]
    return agent.parse_config(agent.pack(car))
...
if __name__ == "__main__":
    agent = nsgaii_agent()
    try:
        print("trying generate car")
        print(generate_car(agent))
        print("trying nparr2carr")
        arr = np.random.rand(15)
        print(np_arr2car(arr,agent))
    except Exception as e:
        traceback.print_exc(file=sys.stdout)    
        print("problem", e)
