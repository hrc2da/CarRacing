...
class DACarRacingRandomStart(gym.Env):
    metadata = {
    ...
    def seed(self, seed=None):
        self.np_random, seed = seeding.np_random(9649348844343419969)
        print("seed", seeding.np_random(seed))

        return [9649348844343419969]
    ...
    def _create_track(self):
        CHECKPOINTS = 6
        try:
            with open(os.path.join(os.getcwd(),self.track_filename), 'rb') as trackfile:
                track, road_tiles, borders = pkl.load(trackfile)
                self.road = []
                for i in range(len(track)):
                    road1_l, road1_r, road2_l, road2_r = road_tiles[i]
                    t = self.world.CreateStaticBody( fixtures = fixtureDef(
                        shape=polygonShape(vertices=[road1_l, road1_r, road2_l, road2_r])
                        ))
                    t.userData = t
                    c = 0.01*(i%3)
                    ...
                    self.road.append(t)
                    if len(borders[i]) > 0:
                        b, beta1, beta2 = borders[i]
                        side = np.sign(beta2 - beta1)
                        self.road_poly.append(( b, (1,1,1) if i%2==0 else (1,0,0) ))
            ...
            print("Failed to load track, generating new one.")
            # Create checkpoints
            checkpoints = []
            ...
            for i in range(len(track)):
                ...
                road_tiles.append([road1_l, road1_r, road2_l, road2_r])
                t = self.world.CreateStaticBody( fixtures = fixtureDef(
            ...
            with open(os.path.join(os.getcwd(),self.track_filename), 'wb+') as trackfile:
    ...
    def reset(self, config={},display=None):
        ...
        start_pose = np.random.randint(0, 114)
        print("resetting to: ", str(start_pose))
        while True:
            success = self._create_track()
            if success: break
            print("retry to generate track (normal if there are not many of this messages)")
        if(config):
            self.car = Car(self.world, *self.track[start_pose][1:4], **config)
        else:
            self.car = Car(self.world, *self.track[start_pose][1:4])
    ...
    def step(self, action,display=None):
        ...
        if self.car: self.car.step(1.0/FPS)
