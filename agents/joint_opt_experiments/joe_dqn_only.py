...
train_dir = 'joe_dqn_only_reset_at_500'
'''
50 replay frequency
250 warmup
0 LCB design steps
2 iterations of 500 policy steps
'''
...
if __name__=="__main__":
    ...
    x0 = [config]
    y0 = np.array([result])
    rewards = policy_step(config2car(config),499)
    for i in range(1):  
        ...
        # results = design_step(x0,y0,acq_func='LCB',iters=100) #,acq_func=acq_func,kappa=4)
        # config = results.x
        ...
        # else:
            ...
            # import pdb; pdb.set_trace()
        rewards = policy_step(config2car(config),500)
