...
driver = DQNAgent(num_episodes=15, replay_freq=50, lr=0.001, model_name = '/home/dev/scratch/cars/carracing_clean/agents/pretrained_drivers/avg_dqn_scratch_driver0.h5', train_dir = 'train_logs_adaptive_random_track_scratch_takeover')
...
def design_step(x0,y0,iters=15,seed=42, acq_func="gp_hedge", kappa='1.96'):
    ...
    res = gp_minimize(test_drive, designbounds, acq_func=acq_func, n_calls=iters, x0=x0, y0=y0, n_random_starts=5, random_state=seed, n_jobs=8, kappa=kappa)
    return res
...
if __name__=="__main__":
    ...
    x0 = [config]
    y0 = np.array([result])
    rewards = policy_step(config2car(config),1)
    for i in range(100):  
        ...
        results = design_step(x0,y0,iters=10,acq_func=acq_func,kappa=4)
        config = results.x
        ...
        # else:
            ...
            # import pdb; pdb.set_trace()
        rewards = policy_step(config2car(config),10)
