...
driver = DQNAgent(num_episodes=15, replay_freq=50, lr=0.001, train_dir = 'train_logs_adaptive_random_track_MAB')
...
if __name__=="__main__":
    ...
    for i in range(100):  
        ...
        action = bandit_pull(Q_vals)
        N_pulls[action] += 1
        mask_indices = get_bandit_mask(action)
        results = design_step(x0,y0,iters=20,acq_func=acq_func,kappa=4)
        config = results.x
