# just so I can run two studies at once; this is the same as the other one.
...
vid_path = 'train_logs_video_dump_50rf_testing_jihyun/vids'
# make the log directory before running
driver = DQNAgent(num_episodes=15, replay_freq=50, lr=0.001, train_dir = 'train_logs_video_dump_50rf_testing_jihyun',vid_path=vid_path)
...
def reset_driver_env():
    global driver
    env = gym.make('CarRacingRandomTrack-v1')
...
def test_drive(config, video_path=None):
    global driver
    ...
    reward = -driver.play_one(eps=0.1,train=False,video_path=video_path)[0]
    driver.env.close()
    return reward
...
if __name__=="__main__":
    ...
    driver.model.save(vid_path+"/warmup.h5")
    ...
    for i in range(100):  
        ...
        for i in range(NUM_TEST_DRIVES):
            test_drive(current_config,video_path=f'test_drive_{i}_{uuid.uuid4().hex}.mp4')
            reset_driver_env()
        ...
        # else:
            ...
            # import pdb; pdb.set_trace()
        rewards = policy_step(config2car(config),20)
